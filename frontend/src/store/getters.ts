...
export const getters = {
  ...
  oneQueryRequest: (state: MainState) => (queryRequestId: number) => {
    const filteredQueryRequests = state.queryRequests.filter(
      (queryRequest) => queryRequest.id === queryRequestId,
    );
  ...
  accessGrants: (state: MainState) =>
    state.accessGrants ? state.accessGrants : [],
  oneAccessGrant: (state: MainState) => (accessGrantId: number) => {
    const filteredAccessGrants = state.accessGrants.filter(
  ...
  accessReceipts: (state: MainState) =>
    state.accessReceipts ? state.accessReceipts : [],
  oneAccessReceipt: (state: MainState) => (accessReceiptId: number) => {
    const filteredAccessReceipts = state.accessReceipts.filter(
      (accessReceipt) => accessReceipt.id === accessReceiptId,
    );
