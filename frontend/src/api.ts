...
export const api = {
  ...
  async getQueryRequestsAccesses(token: string) {
    return axios.get<IQueryRequestsAccess[]>(
      `${apiUrl}/api/v1/queries/requests_access/`,
      this.authHeaders(token),
    );
  },
  async updateQueryRequestsAccess(
    token: string,
    queryRequestsAccessId: number,
    data: IQueryRequestsAccessUpdate,
  ) {
    return axios.put(
      `${apiUrl}/api/v1/queries/requests_access/${queryRequestsAccessId}`,
      data,
      this.authHeaders(token),
    );
  },
  ...
  async updateDataset(token: string, datasetId: number, data: IDatasetUpdate) {
    return axios.put(
      `${apiUrl}/api/v1/datasets/${datasetId}`,
      data,
      this.authHeaders(token),
    );
  },
  ...
  async getAccesses(token: string) {
    return axios.get<IBothAccess>(
      `${apiUrl}/api/v1/accesses/`,
      this.authHeaders(token),
    );
  },
  async getQueries(token: string) {
    return axios.get<IQuery[]>(
      `${apiUrl}/api/v1/queries/`,
      this.authHeaders(token),
    );
  },
  async updateQuery(token: string, queryId: number, data: IQueryUpdate) {
    return axios.put(
      `${apiUrl}/api/v1/queries/${queryId}`,
      data,
      this.authHeaders(token),
    );
  },
  async updateAccessGrant(
    ...
    return axios.put(
      `${apiUrl}/api/v1/accesses/${accessId}`,
      data,
      this.authHeaders(token),
    );
  },
